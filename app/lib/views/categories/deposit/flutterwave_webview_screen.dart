...
class FlutterWebPaymentScreen extends StatelessWidget {
  FlutterWebPaymentScreen({
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarWidget(
        text: Strings.flutterwavePayment.tr,
        onTap: () {
  ...
  _bodyWidget(BuildContext context) {
    var paymentUrl = controller.automaticPaymentFlutterwaveModel.data.url;
    ...
    return InAppWebView(
      ...
      onLoadStop: (controller, url) {
        ...
        if (url.toString().contains('flutterwave/payment/success/')) {
          StatusScreen.show(
              context: context,
